...
CLIENT_PATH = "pure.rest.client.purestorage"
...
class TestBase(object):
    ...
    def setup_method(self, __):
        ...
        self.other_rest_versions = ["0.1", "1.1", "1.0", "1.3"]
...
class TestRequest(TestArrayBase):
    ...
    def make_call(self, method=None, path=None, data=None, cookies=None):
        method = method or self.method
        path = path or self.full_path
        data = data or self.data_json
        cookies = cookies or self.cookies
        return mock.call(method, path, data=data, headers=self.headers,
